...
**Integration hub**: `Viewport` (`canvas/viewport.ts`) orchestrates everything — creates canvas + DOM layers, Camera, InputHandler, ElementStore, NoteEditor, HistoryStack, HistoryRecorder, and the render loop. It has been decomposed into focused collaborators: `SelectionOps`, `GridController`, `ViewportInteractions` (pointer/double-tap/drag-drop/arrow-label/editing), and the keyboard concern lives in `KeyboardHandler` (split out of `InputHandler`). Per-element-type canvas rendering lives in `elements/renderers/` behind a dispatch in `ElementRenderer`.
