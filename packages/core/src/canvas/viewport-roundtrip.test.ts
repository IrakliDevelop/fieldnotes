...
describe('Viewport save/load roundtrip', () => {
  ...
  it('dual-writes fog to both legacy and extensions fields', () => {
    const v1 = new Viewport(container, {
      fog: { color: '#0b1020' },
    });
    ...
    expect(state.extensions?.fog?.data).toEqual(state.fog);

    v1.destroy();
  });

  it('loadState prefers extensions.fog over legacy fog', () => {
    const v1 = new Viewport(container, {
      fog: { color: '#0b1020' },
    });
    ...
    // Tamper: put different fog in legacy field, keep extensions.fog intact.
    const legacyFog = state.fog;
    expect(legacyFog).toBeDefined();
    const tamperedLegacy = {
      ...legacyFog,
      definition: { ...legacyFog.definition, cellSize: 99 },
    };

    const v2 = new Viewport(container, {
      fog: { color: '#0b1020' },
    });
    v2.loadState({ ...state, fog: tamperedLegacy });

    // Should use extensions.fog (cellSize: 4), not the tampered legacy (cellSize: 99).
    const loaded = v2.fog.getState();
    expect(loaded).toBeDefined();
    expect(loaded?.definition.cellSize).toBe(4);

    v2.destroy();
  });

  it('loadState falls back to legacy fog when no extensions', () => {
    const v1 = new Viewport(container, {
      fog: { color: '#0b1020' },
    });
    ...
    // Remove extensions to simulate loading from an old client.
    const { extensions: _extensions, ...legacyOnly } = state;
    void _extensions;

    const v2 = new Viewport(container, {
      fog: { color: '#0b1020' },
    });
    v2.loadState(legacyOnly as typeof state);
  ...
  it('full roundtrip through JSON preserves fog via extensions', () => {
    const v1 = new Viewport(container, {
      fog: { color: '#0b1020' },
    });
    ...
    const loaded = v2.fog.getState();
    expect(loaded).toBeDefined();
    expect(loaded?.definition.bounds).toEqual({ x: 0, y: 0, w: 256, h: 256 });
    expect(loaded?.definition.cellSize).toBe(4);
